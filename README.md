...
1. **Observes** the render queue and artist workload from Postgres
2. **Reasons** about which jobs are genuinely at risk of missing their deadline, and why, using Gemini
...
- **Observability:** Grafana Cloud (Postgres data source + annotation API)
...
4. Create a Grafana Cloud service account token with `annotation:write` and put the
   stack URL + token in the backend `.env` as `GRAFANA_URL` / `GRAFANA_API_KEY` — every
   high-severity MIRAI alert then annotates the dashboard live (`backend/grafana.py`)
5. Grab a public dashboard/panel embed link and set it as `VITE_GRAFANA_PANEL_URL` in
   the frontend
